...
type NodeConfig struct {
	...
	GroupPassword        string                     `comment:"Optional password that restricts end-to-end overlay sessions to nodes\nthat use the same value. Choose a strong, high-entropy password. When\nset, this node cannot access public services or nodes that do not use\nthe password. This is not a firewall: it does not restrict peer links,\nhide peer connections, or prevent group traffic from being routed through\nnon-member nodes. The handshake does not use a slow password hash, so weak\npasswords can be vulnerable to offline guessing. Leave empty for normal\npublic-overlay compatibility."`
...
func (cfg *NodeConfig) ReadFrom(r io.Reader) (int64, error) {
	...
	n := int64(len(conf))
	// An empty document has no configuration value. Report it as invalid input
	// instead of silently replacing the receiver with generated defaults.
	if len(bytes.TrimSpace(conf)) == 0 {
		return n, fmt.Errorf("configuration is empty")
	}
